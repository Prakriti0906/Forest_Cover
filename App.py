...
# REMOVED: The create_mask_image function is no longer needed as the map is not displayed.

# --- Main Analysis Function ---
def analyze_forest_cover(image_bytes, model, patch_size, threshold, progress_bar, status_text):
    """
    Processes a satellite image to calculate forest cover.
    """
    try:
        with rasterio.open(image_bytes) as src:
            ...
            for ji, window in src.block_windows(1):
                ...
                padded_block = np.pad(block_data_preprocessed, [(pad_width, pad_width), (pad_width, pad_width), (0, 0)], mode='constant')
                ...
                predictions = model.predict(patches_np, batch_size=256, verbose=0)
                class_labels = (predictions > threshold).astype(np.uint8)
                
                output_map_block = class_labels.reshape(block_h, block_w)
                full_prediction_map[window.row_off:window.row_off + window.height, 
                                    window.col_off:window.col_off + window.width] = output_map_block
...
with st.sidebar:
    ...
    # The confidence slider is here to control the calculations
    ...
    # Color picker is removed as it's no longer used
    st.markdown("---")
...
if uploaded_file is not None:
    # We still use tabs for clean organization
    ...
    if st.button("Run Analysis", type="primary"):
        ...
        if model:
            ...
            # This container will hold the progress bar
            ...
            progress_container.empty() # Clear the progress bar after completion
            ...
            if forest_pixels is not None:
                ...
                with summary_display.container():
                    ...
                    # Using a fixed color for the chart now
                ...
                # --- Populate the second tab with details ---
                with tab2:
