        ...
        with rasterio.open(image_bytes) as src:
            ...
            for ji, window in src.block_windows(1):
                ...
                padded_block = np.pad(block_data_preprocessed, 
                                      ...
                                      mode='reflect') # 'reflect' is often a good choice to handle edges
                ...
                for y in range(block_h):
                    for x in range(block_w):
                        ...
                        if len(patches) == BATCH_SIZE or (y == block_h - 1 and x == block_w - 1):
                            if not patches:
                                continue
                            ...
                            # Assign predictions back to the block_predictions map
                            patch_idx = 0
                            for py in range(y - len(patches) // block_w + 1, y + 1):
                                for px in range(block_w):
                                    if patch_idx < len(class_labels):
                                        block_predictions[py, px] = class_labels[patch_idx]
                                        patch_idx += 1
